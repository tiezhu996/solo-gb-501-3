...
- 日期严格校验：`trainedAt`、`validUntil` 必须是 `YYYY-MM-DD` 格式的真实日历日期（拒绝 `2026-02-30`、`2026-1-5`、非字符串等），且 `validUntil` 晚于 `trainedAt`；发证与续期时非法日期一律 400 拒绝。历史遗留的非法日期资质在开班校验时同样被拦截并提示更正。
...
```bash
node verify.js   # 28 项断言：正常日期 / 非法日期 / 有效期边界 / 原有三条流程
```
