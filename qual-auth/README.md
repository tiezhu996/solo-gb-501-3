...
```bash
node verify.js   # 28 项断言，可重复运行
```

验证脚本在系统临时目录生成**干净数据副本**，用 `QUAL_AUTH_DATA` + 独立端口拉起专属实例，全部测试数据现场创建；「存量脏数据」用例通过停服→注入非法记录→重启来模拟老版本写入的历史数据；运行前后对正式库 `data/db.json` 做哈希比对，断言零改动。不依赖也不污染正式数据，重复运行结果一致。
